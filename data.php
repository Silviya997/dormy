    ...
    if ($statement->rowCount() == 1) {
        ...
        if($stt->rowCount() == 1) {
            ...
            if($stateOfroom != 'outOforder') {
                ...
                $row=$stm->fetch();
                if($stm->rowCount() == 1) {
                    array_push($errors, 'Faculty No already exist in database .');  ?> <div class="row" style="margin-top:1rem"><center><a href="roomSearch.php?action=search">Search for a room</a></div></center> <?php 
            ...
            array_push($errors, "This room is out of order!");
            } 

        } else {
        array_push($errors, 'Room with this number do not exist.');
